...
def build_paraview_sources(
    ...
    *,
...
) -> tuple[pv.PolyData, pv.PolyData, dict[str, int]]:
    pores = pores.copy()
    ...
    for row in internal.itertuples(index=False):
        ...
        radius = max(float(row.throat_radius_m) / voxel_size_m * throat_radius_scale, min_throat_radius_vox)
...
def bake_single_file_geometry(
    ...
    *,
    ...
    baked = pv.MultiBlock([tubes, spheres]).combine(merge_points=False)
...
def readme_text(metadata: dict) -> str:
    return "\n".join(
        [
            ...
            "",
            "1. Open ParaView.",
            "2. Use `Tools > Python Shell > Run Script...` and select `open_paraview_red_sphere_blue_tube_scene.py`.",
            "3. The script loads the lightweight VTP sources, applies Sphere Glyphs for pores and Tube filters for throats, sets red/blue colors, enables the 3-D grid axes, and tries to save a `.pvsm` state plus a preview PNG.",
            "",
            "If you only use `File > Open`, open the two `.vtp` files manually; then apply `Tube` to throats and `Glyph` with Sphere to pores using the radius arrays.",
            "",
            "## Files",
            "",
            ...
            f"- Direct-open baked VTU: `{metadata.get('baked_single_file_vtu')}`",
...
def export_package(
    ...
    *,
    ...
    baked_vtu = out_dir / f"{prefix}_red_sphere_blue_tube_direct_open.vtu"
    ...
    metadata = {
        ...
        "baked_single_file_vtu": str(baked_vtu) if write_baked else None,
        "baked_single_file_note": (
            "Open this one VTU directly in ParaView. It contains pre-baked red pore sphere surfaces "
            "and blue throat tube surfaces with cell RGB colors."
            if write_baked
            else None
        ),
        ...
        **counts,
        "notes": [
            ...
            "When baked_single_file_vtu is present, it can be opened directly as a single ParaView file.",
    ...
    if write_baked:
        ...
        baked.save(baked_vtu, binary=True)
        metadata["baked_single_file_size_bytes"] = int(baked_vtu.stat().st_size)
